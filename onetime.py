...
def main():
    ...
    txt_1.pack()
    lab_2 = Label(fm_1, text="在下方输入密钥（请保证它只被使用一次且长度与加密字符相等）", font="songti 16 bold")
    lab_2.pack()
    ...
    def ont():
        ...
        if len(key) != len(text):
            showwarning("警告", "密钥长度应与加密字符长度相等！\n（注：可在工具菜单中创建限定长度的随机密钥）")
            return
        encr_list = []
        key_index = 0
        key = key.upper()
    ...
    btn_1 = Button(fm_2, text="加密", command=ont)
...
def main_2():
    ...
    btn_1 = Button(fm_2, text="解密", command=ont)
    btn_1.grid(row=1, column=1, padx=100, ipadx=10, ipady=10)
    btn_2 = Button(fm_2, text="退出", command=win.destroy)
    btn_2.grid(row=1, column=2, padx=100, ipadx=10, ipady=10)
    txt_2.bind("<Button-3>", copy)
